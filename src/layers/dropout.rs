...
impl Layer for Dropout {
    ...
    fn backward(&mut self, d_value: &Array2<f64>) -> Array2<f64> {
        ...
        // dropout layers itself (typically layers's output) divided by the keep_prob value. If that
        // Bernoulli output is a zero, the forward function is zero.
        //
        // Dropout(z) = z * mask / keep_prob
        // d(Dropout(z))/dz = mask / keep_prob
        // z represents the layers's output and obvious the input to the dropout function.
        d_value * self.mask.as_ref().expect("mask was not set. Please run the forward pass first.") / self.keep_prob
    }
